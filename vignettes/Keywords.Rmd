...
The _T_-matrix file format is inferred from the filename extension, which must be either '.txt' or '.h5'. A plain text T-matrix must be formatted as follows:
...
The alternative `.tmat.h5` format is described at [https://github.com/tfp-photonics/tmatrix_data_format](https://github.com/tfp-photonics/tmatrix_data_format). Due to the intricacies of input/output in Fortran, TERMS imposes further restrictions:

- only `vacuum_wavelength` in nanometres is supported, and the wavelengths much match exactly the input file
- TERMS does not check the embedding medium, so it also the user’s responsibility to ensure they are identical
