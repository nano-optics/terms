module multiscat
   ...
   real(8) :: rtol_G = tiny1
   !
   character(len=64) :: tfilename_G = 'tmat_col.txt'
   !
   !---------------------------------------------------
...
!if(allocated(inc)) deallocate(inc)
   ...
   subroutine calcOAprops(Tmat, sigOA, verb_, rtol_)
      ...
      !
      ! For unconverged cross-sections, circular dichroism will be summed
      ! over all n <= nmax.
      if (nconv(1) == 0) nconv(1) = nmax
      if (nconv(2) == 0) nconv(2) = nmax
      !
      do i = 1, maxval(nconv) ! order
         !
         !write(*,*) 'nconv=', nconv
         !
         rdum = 0
         ...
         if (i <= nconv(2)) then
            do j = -i, i  ! degree
               p = i*(i + 1) + j
               rdum(2) = rdum(2) + & ! CD
                         dot_product(Mat(1:pmax, p), Mat(1:pmax, p)) + &
                         dot_product(Mat(1:pmax, pmax + p), Mat(1:pmax, pmax + p)) - &
                         dot_product(Mat(pmax + 1:2*pmax, pmax + p), &
                                     Mat(pmax + 1:2*pmax, pmax + p)) - &
                         dot_product(Mat(pmax + 1:2*pmax, p), Mat(pmax + 1:2*pmax, p))
            end do
   ...
   subroutine contractTmat(Tin, scatXYZR, rtr, Tout, verb_, mack_)
      ...
      !endif
      !
      if (dumpTmatCol_G) call dumpTmat(tmat=Tout, &
...
!******************************************************************************
   ...
   subroutine calcOaLDOC(pol_type, r, geometry, TIJ, Or_OC, verb_)
      !------------------------------------------------------------------
      ...
      !============================================================
      use swav, only: calcVSWs, calcVTACs, xyz2rtp, calcVTrtp2xyz
      !---------------------------------------------------
      ! Start of variable declarations.
      !---------------------------------------------------
      ! Passed variables:
      ...
      complex(8), allocatable :: t11(:, :), t22(:, :), t33(:, :)
      ...
      allocate (scatXYZR(3, nscat), vtacsKL(size(TIJ, 1), size(TIJ, 2)))
...
!---------------------------------------------------------
            ...
            !  if (verb > 1) then
            !    write(*,'(i6,12x,A, 30x)', advance='NO')  n ,'-out'
            !    call cpu_time(t1)
            !    call system_clock(toc,count_rate=tps)
            !     if (verb == 2) then
            !        write(*,'(2(1x,es10.3E2))') &
            !        t1-t0, dble(toc-tic)/dble(tps)
            !     else
            !        write(*,'(2(1x,es10.3E2))', advance='NO') &
            !        t1-t0, dble(toc-tic)/dble(tps)
            !        if (LP) then
            !           write(*, '(2(1x, ES24.17E2))') Or_OC(n,1), Or_OC(n,6)
            !        else
            !           write(*, '(1(1x, ES24.17E2))') Or_OC(n,1)
            !        endif
            !      endif
            !  endif
